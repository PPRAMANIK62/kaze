...
- Built-in tools: `read_file` (with path validation, size limits, binary detection), `glob` (pattern matching with project root containment), `grep` (regex content search with file filtering and match limits)
